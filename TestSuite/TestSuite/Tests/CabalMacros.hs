...
test_MinVersionForDependency env = withAvailableSession' env (withOpts ["-XCPP"]) $ \session -> do
...
test_MinVersionForNonDependency env = withAvailableSession' env (withOpts ["-XCPP"]) $ \session -> do
    updateSessionD session update 1
    assertNoErrors session
    let update2 = updateCodeGeneration True
    updateSessionD session update2 1
...
test_checkCabalMacroDefined env = withAvailableSession' env (withOpts ["-XCPP"]) $ \session -> do
    macros <- getCabalMacros session
...
test_includeExternalMacros env = withAvailableSession' env (withOpts ["-XCPP"]) $ \session -> do
    macros <- getCabalMacros session
    assertBool "M with cabal macro exe output" (not $ L.null macros)
    updateSessionD session (update <> updateCodeGeneration True) 1
    assertNoErrors session
