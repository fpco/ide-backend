...
test_codeGenOn env = withAvailableSession' env (withOpts ["-XTemplateHaskell"]) $ \session -> do
...
test_buildExe env = withAvailableSession' env (withOpts ["-XTemplateHaskell"]) $ \session -> do
    withCurrentDirectory "test" $ do
      (originalUpdate, lm) <- getModulesFrom session "TH"
      let update = originalUpdate <> updateCodeGeneration True
      updateSessionD session update (length lm)
...
test_TH env = withAvailableSession' env (withOpts ["-XTemplateHaskell"]) $ \session -> do
    withCurrentDirectory "test" $ do
      (originalUpdate, lm) <- getModulesFrom session "TH"
      let update = originalUpdate <> updateCodeGeneration True
      updateSessionD session update (length lm)
